...
class OrderModel extends ParentModel {
    ...
    public function getOrder($id = null, $data = array()) {
        ...
        if ($id != null) {
            ...
            cust.address AS customer_address, cust.address_bis AS customer_address_bis, cust.zipcode AS customer_zipcode, cust.city AS customer_city, op.price_ttc, p.name AS product_name, p.description. cat.name AS categorie_name,
        ...
        $dbQuery->select("$this->table.*, g.short_description, cust.name, cust.surname, cust.society_name, op.price_ttc");
        if (!empty($data)) {
            $dbQuery->where($data);
        }
        return $dbQuery->get()->getResultObject();
    ...
    public function getOrderByCustomerId($customer_id, $order_id = null) {
        $dbQuery = $this->db->table($this->table);
        $dbQuery->join("customer AS cust", "$this->table.customer_id = cust.id", "inner");
        if (!is_null($order_id)) {
            $dbQuery->select("$this->table.*, op.price_ttc, p.name AS product_name, p.description, cat.name AS categorie_name")
                ->join("ordered_product AS op", "$this->table.$this->primaryKey = op.ordered_id", "inner")
                ->join("product AS p", "op.product_id = p.id", "left")
                ->join("categorie AS cat", "p.categorie_id = cat.id", "inner");
            $dbQuery->where("$this->table.$this->primaryKey", $order_id);
            return $dbQuery->get()->getResultObject();
        } else {
            $dbQuery->select("$this->table.*, op.price_ttc");
        }
        $dbQuery->where("$this->table.customer_id", $customer_id);
        return $dbQuery->get()->getResultObject();
    }
